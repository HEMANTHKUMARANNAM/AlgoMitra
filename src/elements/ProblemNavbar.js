...
const ProblemNavbar = ({ toggleMode, activeMode, setlan, lan , nextQuestionUrl }) => {
  ...
  useEffect(() => {
    ...
    if (!user || !user.uid) {
      ...
      setStatusIcon(null); // Clear status icon if user is not authenticated
      return;
    }
    ...
    const unsubscribe = onValue(dataRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        if (data === true) {
          setStatusIcon(<FaCheck style={{ color: "" }} />);
        } else {
          setStatusIcon(<FaTimes style={{ color: "" }} />);
        }
      } else {
    ...
    >
      <Container fluid>
        ...
        <div className="d-flex align-items-center">
          ...
          <div className="ms-3 d-flex align-items-center">
            {statusIcon && (
              <Badge pill bg={statusIcon.props.style.color === "green" ? "success" : "danger"} className="ms-2">
                {statusIcon}
              </Badge>
            )}
          </div>
